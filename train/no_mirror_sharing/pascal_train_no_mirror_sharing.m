function model = pascal_train_no_mirror_sharing(cls, n, note)
% Train a model.
...
try
  ...
  for i = 1:n
    ...
    [indsA, indsB] = lrsplit(m, spos{i});
    % Train asymmetric root filter on one of these groups
    mA = train(m, spos{i}(indsA), neg_large, true, true, 1, 1, ...
               max_num_examples, fg_overlap, 0, false, ...
               ['lrsplit1A_' num2str(i)]);
    mB = train(m, spos{i}(indsB), neg_large, true, true, 1, 1, ...
               max_num_examples, fg_overlap, 0, false, ...
               ['lrsplit1B_' num2str(i)]);

    models{i} = model_merge({mA, mB});
